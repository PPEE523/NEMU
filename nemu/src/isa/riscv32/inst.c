...
#define INSTPAT_MATCH(s, name, type, ...) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 1100111", jalr, I, s->dnpc = (src1 + imm) & ~1);
  ...
  INSTPAT("??????? ????? ????? 011 ????? 0010011", sltiu,I, R(rd) = (src1 < (uint32_t)imm ? 1 : 0));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 0110011", mul,  R, R(rd) = (word_t)((int32_t)src1 * (int32_t)src2));
  INSTPAT("0000001 ????? ????? 100 ????? 0110011", div,  R, R(rd) = (src2 == 0 ? (word_t)-1 : (int32_t)src1 / (int32_t)src2));
  INSTPAT("0000001 ????? ????? 110 ????? 0110011", rem,  R, R(rd) = (src2 == 0 ? src1 : (int32_t)src1 % (int32_t)src2));
  INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh, R, {int64_t res = (int64_t)(int32_t)src1 * (int64_t)(int32_t)src2;R(rd) = res >> 32;});
  INSTPAT("0000001 ????? ????? 011 ????? 0110011", mulhu, R, {uint64_t res = (uint64_t)(uint32_t)src1 * (uint64_t)(uint32_t)src2;R(rd) = res >> 32;});
  INSTPAT("0000001 ????? ????? 010 ????? 0110011", mulhsu, R, {int64_t res = (int64_t)(int32_t)src1 * (uint64_t)(uint32_t)src2;R(rd) = res >> 32;});
  INSTPAT("0000001 ????? ????? 111 ????? 0110011", remu  , R, R(rd) = (src2 == 0 ? src1 : (uint32_t)src1 % (uint32_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 0110011", divu, R, R(rd) = (src2 == 0 ? (word_t)-1 : (uint32_t)src1 / (uint32_t)src2));

  /* trap */

  INSTPAT("0000000 00001 00000 000 00000 1110011", ebreak, N, nemu_state.state = NEMU_QUIT;);
